            ...
            {
                Artista = gruppo.Key,
                MinValutazione = gruppo.Min(x => x.Quotazione),
                MaxValutazione = gruppo.Max(x => x.Quotazione),
                MediaValutazione = gruppo.Average(x => x.Quotazione)
            })
                        .ToList();
                        foreach (var statistica in opereJoin)
    {
        Console.WriteLine($"Artista: {statistica.Artista}");
        Console.WriteLine($"   Valutazione Minima: {statistica.MinValutazione}");
        Console.WriteLine($"   Valutazione Massima: {statistica.MaxValutazione}");
        Console.WriteLine($"   Valutazione Media: {statistica.MediaValutazione:F2}");
    }
                        // foreach(var v in opereJoin)
                        // {
                        //     var minQuota=decimal.MaxValue;
                        //     var maxQuota=decimal.MinValue;
                        //     Console.WriteLine($"{v.Key}");
                        //     foreach(var t in v)
                        //     {
                        //         Console.WriteLine(t.Quotazione);
                        //         if(t.Quotazione<minQuota)
                        //         {
                        //             minQuota=t.Quotazione;
                        //         }
                        //         if (t.Quotazione>maxQuota)
                        //         {
                        //             maxQuota=t.Quotazione;
                        //         }
                        //     }
                        //     Console.WriteLine($"quotazione minima di {v.Key}: {minQuota} e quotazione massima : {maxQuota}");
                        // }
